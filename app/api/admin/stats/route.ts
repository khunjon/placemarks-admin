  ...
  try {
    ...
    if (!stats) {
      ...
      const fallbackStats = {
        ...
        publishers_count: new Set(curatedLists.map((list) => list.publisher_name).filter(Boolean)).size,
        location_scopes_count: new Set(curatedLists.map((list) => list.location_scope).filter(Boolean)).size,
        avg_places_per_list: 0,
        most_recent_update: curatedLists.length > 0 ? 
          curatedLists.reduce((latest: string, list) => 
